...
class RocketDataCacheDelegate: CacheDelegate {
     ...
     */
    fileprivate let cache = PINDiskCache(name: "EmemberGlobal")
    ...
    func modelForKey<T : SimpleModel>(_ cacheKey: String?, context: Any?, completion: @escaping (T?, NSError?) -> ()) {
        guard let cacheKey = cacheKey,
            ...
            let modelType = T.self as? RocketModel.Type else {
                // NOTE: The last cast is to ensure that we only deal with BaseModels. This allows us to call init(data:)
    ...
    func setModel(_ model: SimpleModel, forKey cacheKey: String, context: Any?) {
        if let model = model as? RocketModel {
            cache.setObject(model.data() as NSCoding, forKey: cacheKey, block: nil)
        } else {
            assertionFailure("In our app, we only want to use RocketData with BaseModels")
        }
    }
