...
public enum TimeZoneAbbreviation: String {
    case GMT7 = "GMT+7"
}
...
public extension Date {
    ...
    init(fromString string: String, basedTimezone: TimeZoneAbbreviation = .GMT7, format:DateFormat, timeZone: TimeZoneHelper = .local)
    {
        if string.isEmpty {
            self.init()
            return
    ...
    func dateAtStartOfDayWithTimeZone(_ abbreviation: String = "UTC") -> Date
    {
        var components = self.components()
        components.hour = 0
        components.minute = 0
        ...
        let tz = Foundation.TimeZone(abbreviation: abbreviation)
    ...
    func dateAtStartOfWeekCurentTime() -> Date
    {
        let flags: Set<Calendar.Component> = [Calendar.Component.year, Calendar.Component.month, Calendar.Component.weekOfYear, Calendar.Component.weekday]
        var components = Calendar.current.dateComponents(flags, from: self)
        components.weekday = Calendar.current.firstWeekday
     ...
     */
    func yearH() -> Int { return self.components().year!  }
    /**
     Returns the month component.
     */
    func monthH() -> Int { return self.components().month! }
    /**
     ...
     */
    func dayH() -> Int { return self.components().day! }
    /**
     Returns the hour component.
     */
    func hourH() -> Int { return self.components().hour! }
    /**
     Returns the minute component.
     */
    func minuteH() -> Int { return self.components().minute! }
    /**
    ...
    func firstDayOfWeek () -> Int {
        ...
        return Date(timeIntervalSinceReferenceDate: interval).dayH()
    }
    /**
    ...
    //MARK: Date To Date
    func toGMT(_ dateFormat: String = DateFormaterKey.serverFormat.rawValue, timeZone: TimeZoneHelper = .local) -> Date {
        //Convert Date to String UTC
        let stringDate = self.toString(.custom(dateFormat), timeZone: timeZone)
        let zone: Foundation.TimeZone
        switch timeZone {
        case .local:
        ...
        let formatter = Date.formatter(dateFormat, timeZone: zone)
        //Set Date Local with string UTC
    ...
    func toDateTimeWith(_ seconds: Int) -> Date {
        return Date(timeInterval: TimeInterval(seconds), since: self)
    }
     ...
     */
    func toString(_ format: DateFormat, timeZone: TimeZoneHelper = .local) -> String
    {
    ...
    func monthToString() -> String {
        let formatter = Date.formatter()
        return formatter.monthSymbols[self.monthH()-1] as String
    }
    ...
    func shortMonthToString() -> String {
        let formatter = Date.formatter()
        return formatter.shortMonthSymbols[self.monthH()-1] as String
    }
    ...
    func veryShortMonthToString() -> String {
        let formatter = Date.formatter()
        return formatter.veryShortMonthSymbols[self.monthH()-1] as String
    }
