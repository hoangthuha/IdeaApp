...
open class AlertVC {
    ...
    open class func alert(_ title: String, message: String) -> UIAlertController {
        return alert(title, message: message, acceptMessage: "alert.okAction".localized(), acceptBlock: {
            // Do nothing
        })
    }
    ...
    @discardableResult
    open class func alert(_ title: String?, message: String?, okTitle: String, okBlock: @escaping () -> ()) -> UIAlertController {
        let alert = UIAlertController(title: title, message: message, preferredStyle: UIAlertControllerStyle.alert)
        let okButton = UIAlertAction(title: okTitle, style: .default, handler: { (action: UIAlertAction) in
            okBlock()
        })
    ...
    open class func showTextFieldError(title: String?, message: String?, textField: UITextField, completion: (() -> Void)?) -> UIAlertController {
        ...
        let okAction = UIAlertAction(title: "a2z.alert.okAction".localized(), style: .default) { (action) in
